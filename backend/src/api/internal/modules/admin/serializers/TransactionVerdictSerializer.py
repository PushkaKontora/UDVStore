...
class TransactionVerdictSerializer(serializers.Serializer):
    transaction_id = serializers.PrimaryKeyRelatedField(queryset=get_requested_transactions(),
                                                        many=False)
    amount = serializers.IntegerField(validators=[MinValueValidator(1)],
                                      required=False, default=0)
    comment = serializers.CharField(max_length=Transaction.DESCRIPTION_LENGTH,
                                    allow_blank=True,
                                    allow_null=True)
