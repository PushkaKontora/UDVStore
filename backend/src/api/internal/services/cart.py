...
def validate_balance(profile: Profile, orders: Iterable[Order]) -> bool:
    return profile.balance >= sum(get_total(order) for order in orders)
...
def pay(profile: Profile, orders: Iterable[Order]) -> List[Transaction]:
    if not validate_amount(orders) or not validate_balance(profile, orders):
        return []
